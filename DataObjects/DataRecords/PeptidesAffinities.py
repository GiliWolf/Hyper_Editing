...
class PeptideAffinity(namedtuple("PeptideAffinity", "peptide_site mhc_allele offset mismatches_sites ligand_length rank"
                                                    " affinity_rank affinity binding_core")):
    """
    ...
    :ivar int offset: Offset of the k-mere from the start of the peptide.
    :ivar list[Site] mismatches_sites: The sites of mismatches from the "canonical" *DNA* seq.
    :ivar int ligand_length: The length of the predicted ligand
    TODO: This is netMHC dependant. rethink.
    :ivar float rank: The rank of the binding
    :ivar AffinitiesEnum affinity_rank: The affinity rank of the peptide core (strong, intermediate or weak).
    :ivar float affinity: The affinity in nM.
    :ivar str binding_core: The predicted binding core.
    """
    __slots__ = ()


class PeptidesAffinities(DataRecord):
    """
    ...
    def __init__(self, source_name):
        ...
        self.__affinities_by_site = dict()
        self.__affinities_by_affinity_rank = dict()
    ...
    def dump_me(self, dump_path):
        ...
        for site, alleles_dict in self.__affinities_by_site.iteritems():
            """:type site Site"""
    ...
    def load_me(dump_path):
        ...
        for line in instance_data[1:]:
            ...
            copy.add_peptide_aff(region, start, end, strand, mhc_class, locus_name, allele_name, offset, ligand_length,
    ...
    def add_peptide_aff(self, region, start, end, strand, mhc_class, locus_name, allele_name, offset, ligand_length,
                        rank,
                        affinity_rank, affinity, binding_core=NA, mismatches_sites=NA):
        # type: (str, int, int, str, str, str, str, int, int, float, AffinitiesEnum, float, str, list) -> None
        """
        This function adds to the affinity dictionary
        :param str region: The chromosome (or transcript name)
        :param int start: start position
        :param int end: end position
        :param str strand: the strand of the site
        :param int offset: the offset of the peptide "window" from the start of th sequence.
        :param int ligand_length: The length of the ligand.
        :param float rank: The rank of the binding
        :param AffinitiesEnum affinity_rank: The affinity rank of the peptide core (strong, intermediate or weak).
        :param float affinity: The affinity in nM.
        :param str binding_core: The predicted binding core.
        :param mismatches_sites: A list of mismatches site contributing to the current peptide sequence,   b
        ...
        pep_aff = PeptideAffinity(peptide_site, mhc_allele, offset, mismatches_sites, ligand_length, rank,
                                  affinity_rank, affinity, binding_core)
        self.__affinities_by_site.setdefault(peptide_site, {}).setdefault(mhc_allele, {}).setdefault(affinity_rank, []). \
            append(pep_aff)
        self.__affinities_by_affinity_rank.setdefault(affinity_rank, {}).setdefault(peptide_site, {}). \
            setdefault(mhc_allele, []).append(pep_aff)

    def get_peptides_affinities(self, dont_filter=True, by_rank=False, mhc_alleles=None, sites=None,
                                affinities_ranks=None):
        """
        Retrieves the sites.
        :param bool dont_filter: A flag. if set will retrieving a simple copy without filtration. Use to save run-time.
        :param bool by_rank: A flag. if set will return affinities by the rank otherwise by site.
        :param list[MHCAllele] mhc_alleles: if given will return only affinities to alleles that match this.
        :param list[Site] sites: if given will return only affinities to these sites.
        :param list[AffinitiesEnum] affinities_ranks: if given will return only affinities with these ranks.
        ...
        ranks_d = convert_params_to_bool_dict(affinities_ranks)
        if by_rank:
            filtered_copy = deep_copy_dict(self.__affinities_by_affinity_rank)
            if dont_filter:
                return filtered_copy
            for rank, sites_dict in filtered_copy.iteritems():
                if affinities_ranks and not ranks_d.get(rank, False):
                    _ = filtered_copy.pop(rank)
                    continue
                for site, alleles_dict in sites_dict.iteritems():
                    if sites and not mhc_alleles_d.get(site, False):
                        _ = sites_dict.pop(site)
                        continue
                    for allele in alleles_dict:
                        if mhc_alleles and not sites_d.get(allele, False):
                            _ = alleles_dict.pop(allele)
                            continue

        else:
            filtered_copy = deep_copy_dict(self.__affinities_by_site)
            if dont_filter:
                return filtered_copy

            for site, alleles_dict in filtered_copy.iteritems():
                if sites and not sites_d.get(site, False):
                    _ = filtered_copy.pop(site)
                    continue
                for allele, aff_rank_dict in alleles_dict.iteritems():
                    if mhc_alleles and not mhc_alleles_d.get(allele, False):
                        _ = alleles_dict.pop(allele)
                        continue
                    for aff_rank in aff_rank_dict:
                        if affinities_ranks and not ranks_d.get(aff_rank, False):
                            _ = aff_rank_dict.pop(aff_rank)
                            continue
        return filtered_copy
