...
class TCGAShacharMirClusteringHelper(RNAQuantificationClustererHelper):
    ...
    def sort_file_type(self, path, filtered, unfiltered):
        ...
        if path.endswith(self.filtered_suffix):
            ...
            if self.unfiltered_found.get(sample_params[PARTICIPANT_NUM] + sample_params[SAMPLE_TYPE] + group, True):
                unfiltered.append(path)
                self.unfiltered_found[sample_params[PARTICIPANT_NUM] + sample_params[SAMPLE_TYPE] + group] = False
