__author__ = 'Hillel'
# =====================version=====================#
# Roni Cohen-Fultheim For FastQC version 0.11.8 (new version)
...
SEQ_DUP_TOT_LINE_RE = re.compile(r"#Total Deduplicated Percentage\t(.*?)\r?\n")
SEQ_DUP_LINE_RE = re.compile(r"(\d.*?)\t(.*?)\r?\t(.*?)\r?\n")
...
def analyse_seq_dup(data, f, sample):
    ...
    header = BASIC_HEADER_FORMAT % ("NonDuplicatedPC", "Total")
    rec[header] = tot_dup
    for line in seq_dup_data:
        # for % left after deduplication
        header = BASIC_HEADER_FORMAT % ("NonDuplicatedPC", line[0])
        rec[header] = line[1]
        # for % of total
        header = BASIC_HEADER_FORMAT % ("DuplicationLevelOfTotalPC", line[0])
        rec[header] = line[2]
    return rec
...
if __name__ == "__main__":
    desc = """Get summary for fastqc analysis, for FastQC version 0.11.8 (new version)"""
