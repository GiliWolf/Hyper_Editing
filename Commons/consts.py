from Commons.data_structs import Site, RefSeq
...
class MismatchesAndRefsEnum(object):
    ...
    @staticmethod
    def complementary_mm_in_format(field):
        # type: (str) -> str
        for mm in MismatchesAndRefsEnum.ALL_MISMATCHES:
            if mm in field:
                return field.replace(mm, MismatchesAndRefsEnum.COMPLEMENTARY_MM[mm])

# endregion

# =====================consts=====================#
KW_ARGS_RE = re.compile("(?:^ *\w*=.*$)|(?:^$)")

MULTI_VALUE_OUTPUT_SEP = "#"
# instead of ":", for example Wilcox: 0.05 -> Wilcox=0.05
KEY_VAL_OUTPUT_SEP = "="
