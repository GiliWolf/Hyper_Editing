from Tools.EditingIndex.DataConverters.converters_commons import pyi_safe_subprocess
...
from Commons.consts import EMPTY_SEQ, ANTISENSE_STRAND, MismatchesAndRefsEnum, UNKNOWN_STRAND
from Commons.data_structs import Site
from Commons.general_functions import get_file_handle
from converters_commons import pyi_safe_subprocess
...
def load_snps_file(snps_file, intersect_first_with='', bedtools_path="bedtools"):
    ...
    if intersect_first_with:
        popen_cmd = INTERSECT_FORMAT % dict(bedtools=bedtools_path, snps_file=snps_file,
                                            intersect_file=intersect_first_with)
        snps_fh = pyi_safe_subprocess(popen_cmd=popen_cmd)
    else:
    ...
    for line in snps_fh:
        ...
        for snp_seq in seqs:
            ...
            if strand == ANTISENSE_STRAND:
                mismatch = MismatchesAndRefsEnum.MISMATCH_TYPE_PARSE[
                    MismatchesAndRefsEnum.COMPLEMENTARY_REF[ref_seq] + MismatchesAndRefsEnum.COMPLEMENTARY_REF[snp_seq]]
            else:
                mismatch = MismatchesAndRefsEnum.MISMATCH_TYPE_PARSE[ref_seq + snp_seq]
