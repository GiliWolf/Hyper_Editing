...
def get_index(refseq_strands, refseq_annotations, samples_configs, summery_dir, pipeline_output_dir, snps,
    ...
    if get_regions_data:
        logging.debug("Initializing Regions Counter For Get Regions Data")
        ...
        for count_type in sorted(empty_counter.iterkeys()):
            ...
            empty_reg_dict[avg_header] = 0
            empty_reg_dict[tot_header] = 0
    ...
    for rec_id, sample in samples.iteritems():
        ...
        group = NA
        for group_p in samples_groups_dict.get(sample_name).keys():
            if group_p[1] <= 1:
                group = group_p[0]
                break
...
def populate_get_region_data_d(annotations, empty_reg_dict,  # manager,
                               region):
    # annotations_dict = add_region_annotations(annotations, region)
    # reg_dict = manager.dict(annotations_dict)
    reg_dict = add_region_annotations(annotations, region)
    reg_dict.update(empty_reg_dict)
    return reg_dict


def add_region_annotations(annotations, region):
...
def generate_per_region_output(get_regions_data_dict, num_of_samples, per_sample_output, per_sample_output_dir,
                               sample_name, sample_path, regions_d, group, empty_reg_dict, refseq_annotations):
    # type: (dict, int, bool, str, str, str, dict, str, dict, dict) -> None
    """
    ...
    :param sample_name: The sample processed.
    :param sample_path: The sample processed path.
    ...
    logging.info("Generating Per Region Data for %s" % sample_name)
    for region, region_data_d in regions_d.iteritems():
        annotations = refseq_annotations[region]
        if per_sample_output:
            init_per_sample_region_dict(annotations, group, per_sample_dict, region,
                                        sample_path,
                                        sample_name)

        per_region_count_dict = get_regions_data_dict.get(region,
                                                          None)  # to overcome python2.7 multiprocessing nested proxy bug
        if None is per_region_count_dict:
            per_region_count_dict = populate_get_region_data_d(
                annotations=annotations,
                empty_reg_dict=empty_reg_dict,
                # manager=manager,
                region=region)
        ...
        if per_sample_output:
            ...
            per_sample_regions_recs.append({k: str(v) for k, v in per_sample_dict.iteritems()})

    logging.info("Done Summing Per Sample Regions Data for %s" % sample_name)

    if per_sample_output:
        logging.info("Creating Per Sample Region Counts Output For %s" % sample_name)
...
def generate_per_region_output_stranded(get_regions_data_dict, num_of_samples, per_sample_output, per_sample_output_dir,
                                        sample_name, sample_path, refseq_annotations, regions_counts, group,
                                        split_by_strand, empty_reg_dict):
    # type: (dict, int, bool, str, str, str, dict, dict, str, bool, dict) -> None
    """
    ...
    :param refseq_annotations: The annotation(s) of each the regions.
    :param num_of_samples: The total number of samples for calculation.
    ...
    logging.info("Summing Per Sample Regions Data for %s" % sample_name)
    ...
    for region, region_data_d in regions_counts[SENSE_STRAND].iteritems():
        ...
        annotations = refseq_annotations[region]
        if per_sample_output:
            init_per_sample_region_dict(annotations, group, per_sample_region_data_dict, region,
                                        sample_path,
                                        sample_name)
            per_sample_region_data_dict.update(get_empty_region_counts())
            # if split_by_strand:
    ...
    for region, region_data_d in regions_counts[ANTISENSE_STRAND].iteritems():
        if per_sample_output and region not in per_sample_regions_output_recs:
            ...
            # per_sample_region_data_dict = per_sample_region_data_dict.copy()
        ...
        for count_type, count in region_data_d.iteritems():
            ...
            if per_sample_output:
                try:
                    ...
                    import pprint
                    logging.exception(pprint.pformat(per_sample_regions_output_recs[region]))
                    raise e
...
def init_per_sample_region_dict(annotations, group, per_sample_region_data_dict, region, sample_path,
                                sample_name):
    per_sample_region_data_dict[GROUP] = group
    per_sample_region_data_dict[SAMPLE] = sample_name
    per_sample_region_data_dict[SAMPLE_PATH] = sample_path
    per_sample_region_data_dict[STRAND_DECIDING_METHOD] = STRAND_DECIDING_STRANDED_DATA
    per_sample_region_data_dict.update(add_region_annotations(annotations, region))


def output_sample_indexes(summery_dir, sample_name, sample_path, refseq_annotations, regions_d, group,
                          all_methods_sample_index_dict, verbose, possible_mismatches):
    # type: (str, str, str, dict, dict, str, dict, bool, list) -> None
    """
    ...
    per_sample_regions_dict[SAMPLE_PATH] = sample_path

    logging.info("Summing Data for %s" % sample_name)
    for region, region_data_d in regions_d.iteritems():
    ...
    logging.info("Done Summing Data for %s" % sample_name)
    ...
    for strand_deciding_method in STRAND_DECIDING_OPTIONS:
        ...
        sample_rec[SAMPLE] = sample_name
        sample_rec[SAMPLE_PATH] = sample_path
...
def output_sample_stranded(summery_dir, sample_name, sample_path, refseq_annotations, regions_counts,
                           group, verbose, possible_mismatches):
    # type: (str, str, str, dict, dict, str, bool, list) -> None
    """
    This methods calc the output for stranded data
    :param summery_dir: The dir to create summery outputs in.
    :param sample_name: The sample processed name.
    :param sample_path: The sample processed path.
    :param refseq_annotations: per region refseq annotation
    :param regions_counts: The processed data of the sample (per region).
    :param group: The sample's group.
    :param bool verbose: A flag. If set, will output a verbose output for the index, including all the counts
    :param list possible_mismatches: what mismatch types to output.
    :return: None
    """
...
def load_and_process_sample(all_methods_sample_index_dict, config_defaults, get_regions_data_dict, refseq_annotations,
                            refseq_strands, group_name, snps, per_sample_output, max_processes, num_of_samples,
                            pipeline_output_dir, summery_dir, sample_name, sample_path, verbose, is_stranded,
                            possible_mismatches, empty_reg_dict):
    # type: (dict, dict, dict, dict, dict,str, dict, bool, int, int, str, str, str, str,  bool, bool, list, dict) -> None
    """
    ...
    :param str group_name: the name of the sample group.
    ...
    try:
        if is_stranded:
            ...
            logging.info("Processing The Data Of %s as Stranded" % sample_name)
            ...
            if get_regions_data_dict is not None:
                generate_per_region_output_stranded(get_regions_data_dict=get_regions_data_dict,
                                                    ...
                                                    empty_reg_dict=empty_reg_dict  # ,manager=manager
                                                    )
        else:
            # get cmpileup path
            count_pileup_file = config_defaults[REGIONS_PILEUP_COUNT_OPTION]
            logging.info("Processing The Data Of %s as Unstranded" % sample_name)
        ...
        logging.exception("Failed Processing of %s!" % sample_name)
    finally:
        if is_stranded:
...
def get_strands_and_counts(refseq_strands, sample_name, snps, max_processes, count_pileup_file, verbose):
    # type: (dict, str, dict, int, str, bool) -> dict
    """
    ...
    :param dict[Site, str] refseq_strands: The most likely strand of the regions according to the annotations (by region).
    :param str sample_name: The sample we're processing now.
    :param dict snps: A dictionary with the SNPs per mismatch type per position.
    :param int max_processes: The maximal number of threads for strands decision
...
def get_stranded_strands_and_counts(refseq_strands, sample_name, snps, count_pileup_file_s_1,
                                    count_pileup_file_s_2):
    # type: (dict, str, dict, str, str) -> dict
    """
    This function parallels (using threading) the processing of the sample's data (counting coverages and mismatches, deciding strands per region)
    :param dict[Site, str] refseq_strands: The most likely strand of the regions according to the annotations (by region).
    :param str sample_name: The sample we're processing now.
    ...
    :return dict: The regions counts and strands
    """
    # Old - sample_name = sample.sample_name
...
def main(root_dir, output_dir, output_dir_summery, genome_resources, log_path, bam_files_suffix,
         ...
         gene_expression_file, reload_pickled_resoures, cds_index, use_refseq_all, get_regions_metadata,
         just_get_configs,
    ...
    :param bool cds_index: If set, will index over the built-in CDS regions instead of the built-in repeats (Alu/SINE B1-2).
    ...
    if not cds_index:
        regions_bed_full_path = builtins_resources[RESOURCES_INI_REGIONS_OPT] if (
                regions_bed == "" and using_builtins_resources) else regions_bed
    else:
        if regions_bed != "":
            logging.warning("CDS Index was Requested According to Parameters, But a Regions BED was Provided!")
...
if __name__ == "__main__":
    ...
    resources_g.add_argument('--reload_pickled_resoures', dest='reload_pickled_resoures', action='store_true',
                             required=False, help="If set, will try to load pre-processed resources saved by "
                                                  "PreloadAEIResources.")
    resources_g.add_argument('--CDS_index', dest='cds_index', action='store_true',
                             required=False, help="If set, will index over the built-in CDS regions instead of"
                                                  " the built-in repeats (Alu/SINE B1-2)")
