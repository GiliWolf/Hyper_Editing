...
def get_common_sites(bed_files, min_fraction_edited=1, min_coverage=10):
    sites_c = Counter()
    mm_c = Counter()
...
def divide_paths(path_groups_csv, path_i, category_is, must_include_paths):
    categories_d = dict()
...
def create_common_rated_sites(path_groups_csv, path_i, categories_is, min_common_fraction, output_dir,
                              must_include_paths, min_coverage):
    categories_d = divide_paths(path_groups_csv, path_i, categories_is, must_include_paths)
    for category, paths in categories_d.iteritems():
        common_sites = get_common_sites(bed_files=paths, min_fraction_edited=min_common_fraction, min_coverage=min_coverage)
