...
from Commons.consts import POSSIBLE_STRANDS, GROUP, SAMPLE, SAMPLE_PATH, MismatchesAndRefsEnum, MISMATCH_TYPE
from Commons.data_structs import RefSeqPosEnum
from Outputs.Outputers.CSVOutputer import CSVOutputer
from Tools.EditingIndex.EditingIndexConsts import SUMMERY_G_STRANDED_INDEXED_EDITED_FORMAT, \
...
def get_index_summery_statistics(index_summery_path, summery_out_path, groups_summery_path):
    ...
    :param str groups_summery_path: The path to the groups summery output.
...
if __name__ == "__main__":
    ...
    desc = """Creates a summed percentile output from the index full counts output"""
    ...
    parser.add_argument('-o', '--output_dir', metavar="output_dir", dest='output_dir', nargs='?', required=False,
                        default=".", help="The root directory for the output (where per sample and per group data "
                                          "will be written)")
